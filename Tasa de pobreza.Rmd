¿Por qué es importante considerar la tasa de pobreza?
...
```{r}
library(rio)
lkCSV="https://github.com/Natalia-Aquize/poverty/raw/master/DP_LIVE_26092020233557523.csv"
...
datafinal=merge(p1p2p3p4,pobreza5,all.x=T,all.y=T)
head(datafinal)
```
Número de NAs de la base de datos del 2011:10
```{r}
sum(is.na(datafinal$"Data 2011"))
```
Número de NAs de la base de datos del 2012:11
```{r}
sum(is.na(datafinal$"Data 2012"))
```
Número de NAs de la base de datos del 2013:10
```{r}
sum(is.na(datafinal$"Data 2013"))
```
Número de NAs de la base de datos del 2014:9
```{r}
sum(is.na(datafinal$"Data 2014"))
```
Número de NAs de la base de datos del 2015:7
```{r}
sum(is.na(datafinal$"Data 2015"))
```
Se escoge la base de datos del 2015 por tener menos cantidad de NAs
```{r}
datafinal$`Data 2011`= NULL
datafinal$`Data 2012`= NULL
datafinal$`Data 2013`= NULL
datafinal$`Data 2014`= NULL
datafinal=na.omit(datafinal)

write.csv(datafinal, "pobreza mundial.csv", row.names=F)
```
